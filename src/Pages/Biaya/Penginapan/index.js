...
import { DeletePenginapan, GetAllDataPenginapan, GetDataPenginapanById } from "Services";
...
export const Penginapan = () => {
    ...
    const fetchDataById = async (id) => {
        try {
            const response = await GetDataPenginapanById(id);
            if (response.data) {
    ...
    const deleteData = async () => {
        try {
            const response = await DeletePenginapan(state.selectedId);
            if (response.data) {
                fetchAllData(1);
